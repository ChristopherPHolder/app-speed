...
const processAuditRequest = Effect.fn('runner.queue.processItem')(function* (
  auditRequest: AuditClaim,
  stateRef: Ref.Ref<QueueState>,
) {
  yield* setBusyState(stateRef);
  yield* Effect.annotateCurrentSpan({ 'audit.id': auditRequest.auditId });
  const executor = yield* AuditExecutor;
  const [duration, exit] = yield* Effect.timed(Effect.exit(executor.execute(auditRequest)));
  ...
  if (Exit.isSuccess(exit)) {
    ...
    yield* completeAuditRun(auditRequest.auditId, auditRequest.kind, { status: 'FAILURE', error }, durationMs);
  }
