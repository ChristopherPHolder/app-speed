...
@Component({
  selector: 'builder-contract-step-fields',
  standalone: true,
  imports: [
    NgTemplateOutlet,
    ReactiveFormsModule,
    ...
    MatOption,
    MatSelect,
    forwardRef(() => ContractStepFieldsComponent),
  ],
  template: `
          ...
          <section class="group-field">
            ...
            <div class="field-row field-row--record-add">
              ...
              <button
                mat-button
                type="button"
                (click)="addRecordEntry(field, asFormRecord(fieldControl), recordKey)"
              >
                Add Entry
              </button>
            </div>
...
export class ContractStepFieldsComponent {
  ...
  protected childControl(field: BuilderFieldContract): AbstractControl {
    const control = this.control().get(contractFieldControlName(field));
  ...
  protected labelFor(field: BuilderFieldContract): string {
    return (
      getContractFieldPresentation(this.variantId(), field.path)?.label ??
      humanizeToken(contractFieldControlName(field) || field.path)
    );
  }

  protected descriptionFor(field: BuilderFieldContract): string | undefined {
    return getContractFieldPresentation(this.variantId(), field.path)?.description;
  }
