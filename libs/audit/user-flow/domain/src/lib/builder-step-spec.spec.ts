...
import { USER_FLOW_AUDIT_BUILDER_STEP_VARIANTS, deriveBuilderStepSpec } from './builder-step-spec';

describe('builder step spec', () => {
  it('exports every current builder-creatable variant', () => {
    expect(USER_FLOW_AUDIT_BUILDER_STEP_VARIANTS.map((variant) => variant.id)).toEqual([
      'waitForElement',
      'waitForExpression',
      'change',
      'click',
  ...
  it('derives specs for every registered variant', () => {
    expect(() => USER_FLOW_AUDIT_BUILDER_STEP_VARIANTS.map(deriveBuilderStepSpec)).not.toThrow();
  });
...
const findVariant = (id: string) => {
  const variant = USER_FLOW_AUDIT_BUILDER_STEP_VARIANTS.find((candidate) => candidate.id === id);
