...
describe('auditBuilderRoutes', () => {
  ...
  const configureRouteTestingModule = async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        provideRouter([{ path: 'audits', children: [{ path: 'user-flow', children: auditBuilderRoutes }] }]),
      ],
    }).compileComponents();
  };
  ...
  it('loads persisted run details when visiting the canonical result URL', async () => {
    ...
    await harness.navigateByUrl('/audits/user-flow/results/run-123', BuilderComponent);
  ...
  it('ignores repeated submit attempts while scheduling is already in flight', async () => {
    ...
    const harness = await RouterTestingHarness.create();
    const builder = await harness.navigateByUrl('/audits/user-flow', BuilderComponent);
  ...
  it('redirects the bare results route to history', async () => {
    ...
    const harness = await RouterTestingHarness.create();
    await harness.navigateByUrl('/audits/user-flow/results');

    expect(TestBed.inject(Router).url).toBe('/audits/user-flow/results/history');
  ...
  it('shows an inline error instead of the draft builder when the result id cannot be loaded', async () => {
    ...
    const harness = await RouterTestingHarness.create();
    await harness.navigateByUrl('/audits/user-flow/results/missing-run', BuilderComponent);
    ...
    const root = harness.fixture.nativeElement as HTMLElement;
    expect(TestBed.inject(Router).url).toBe('/audits/user-flow/results/missing-run');
  ...
  it('navigates submitted drafts to the canonical result URL without draft query details', async () => {
    ...
    const builder = await harness.navigateByUrl(
      `/audits/user-flow?audit-details=${encodeURIComponent(JSON.stringify(DEFAULT_AUDIT_DETAILS))}`,
      BuilderComponent,
    );
    ...
    expect(TestBed.inject(Router).url).toBe('/audits/user-flow/results/scheduled-123');
