...
import type { DeviceType } from '@app-speed/audit/core/domain';
import type { UserFlowAuditStep } from '@app-speed/audit/user-flow/domain';
import { AuditDetails } from '../audit-details';
import { BuilderStepFormGroup, findStepSpec } from '../step-form';

export type StepSelection = string;
export type Step = UserFlowAuditStep | { type: '' };
...
export class StepFormGroup extends BuilderStepFormGroup {
  ...
  constructor(step: Step | UserFlowAuditStep) {
    super();
  ...
  resetStepControls(stepSelection: StepSelection | '', step?: UserFlowAuditStep): void {
    if (this.disabled) {
      return;
    }
...
const stepSelectionFromStep = (step: Step | UserFlowAuditStep): StepSelection | '' => {
  if (step.type === '') {
    return '';
  }
...
const isSelectedStep = (step: Step | UserFlowAuditStep): step is UserFlowAuditStep => step.type !== '';
