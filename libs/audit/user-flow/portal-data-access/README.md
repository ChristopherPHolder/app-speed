# @app-speed/audit/user-flow/portal-data-access

Nx library for the user-flow portal data-access boundary under `libs/audit/user-flow/portal-data-access`.
...
- `src/index.ts`
  - public API surface used by `libs/audit/user-flow/feature-builder`
...
Current source of truth: `libs/audit/user-flow/feature-builder`.

### Strong First Moves

- `src/lib/api/audit-builder-api.service.ts`
  - Direct delegate for `POST /api/audits/user-flow/schedule`
...
4. Keep `libs/audit/user-flow/feature-builder` consuming the exported data-access surface.
