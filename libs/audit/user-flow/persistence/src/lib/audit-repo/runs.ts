...
export const getRunSummaryById = Effect.fn('db.auditRun.getSummaryById')(function* (id: AuditRunId) {
      ...
      (
        await client
          .select({
            ...
            definition: userFlowAuditTemplateTable.definition,
            resultStatus: auditResultTable.status,
          })
          .from(auditRunTable)
          .innerJoin(auditTemplateTable, eq(auditTemplateTable.id, auditRunTable.templateId))
          .innerJoin(userFlowAuditTemplateTable, eq(userFlowAuditTemplateTable.templateId, auditTemplateTable.id))
  ...
  return yield* decodeAuditRunSummaryRecord({
    id: row.id,
    title: resolveAuditTitle(row.definition),
    status: row.status,
