...
const processAuditRequest = (auditRequest: AuditRequest, stateRef: Ref.Ref<QueueState>) =>
  Effect.gen(function* () {
...
const queueLoop = (stateRef: Ref.Ref<QueueState>, idleTimeoutMs: number, idlePollIntervalMs: number) =>
  Effect.gen(function* () {
    while (true) {
  ...
  }).pipe(Effect.withSpan('runner.queue.loop'));
